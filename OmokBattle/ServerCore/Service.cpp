...
void ClientService::Init(int32 count)
{
	for (int i = 0; i < count; i++) {
		...
		if (SocketUtils::CreateListenSocket(socket))
		...
		if (SOCKET_ERROR == ::WSAEventSelect(socket, event, FD_READ | FD_WRITE | FD_CLOSE))
...
void ServerService::Listen()
{
	...
	if (SocketUtils::CreateListenSocket(_listenSocket))
